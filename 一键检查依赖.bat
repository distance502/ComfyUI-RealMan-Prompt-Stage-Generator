...
if not defined PYTHON_CMD (
    ...
    echo [INFO] You can also run dependency_probe.py with your ComfyUI Python manually.
    pause
    exit /b 1
)
...
"%PYTHON_CMD%" %PYTHON_ARGS% "%SCRIPT_DIR%dependency_probe.py" "%COMFY_ROOT%"
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if "%EXIT_CODE%"=="0" (
    echo [DONE] Dependency check passed.
) else (
    echo [DONE] Dependency check finished with issues to review.
)
pause
exit /b %EXIT_CODE%
