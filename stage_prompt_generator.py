...
def _加载阶段模型(kwargs: dict[str, Any]) -> Any:
    source = str(kwargs.get("模型来源", SETTING_DEFAULTS["模型来源"]) or SETTING_DEFAULTS["模型来源"]).strip()
    if source == "仅Skill":
        return None
    if source == "API接口":
...
def _safe_stage_model_source_label(source: str) -> str:
    return source if source in {"仅Skill", "API接口", "本地GGUF"} else (source or "未知来源")
...
_阶段输入参数说明 = {
    "内置上下文长度": "本地 GGUF 的上下文窗口。默认 8192；越大越占内存/显存，普通提示词生成通常不需要超过 16384。",
    "内置GPU层数": "本地 GGUF 的 GPU 卸载层数。-1=尽可能使用 GPU，0=纯 CPU；显存不足时逐步降低。",
...
class QwenTE阶段式提示词生成器:
    ...
    def INPUT_TYPES(cls):
        ...
        required = OrderedDict()
        required["模型来源"] = (模型来源选项, {"default": "仅Skill", "tooltip": "仅Skill=不调用模型；本地GGUF=加载 models/LLM；API接口=使用模型按钮里的 API 配置调用云端或本地兼容接口。"})
        required["内置模型系列"] = (_内置模型系列选项, {"default": "Qwen3.5-VL", "tooltip": "无需连接模型加载器时，阶段提示词节点会按这里的设置直接加载并调用模型。"})
        required["内置主模型"] = (model_list, {"default": model_list[0], "tooltip": "主模型文件（建议 .gguf）放到 ComfyUI/models/LLM/。"})
        ...
        return {
            "required": required,
            "optional": {"qwen模型": ("QWENLLAMA",), "参考图片": ("IMAGE",)},
    ...
    def run(self, qwen模型=None, **kwargs):
        ...
        source = str(settings.get("模型来源", SETTING_DEFAULTS["模型来源"]) or SETTING_DEFAULTS["模型来源"]).strip()
...
class QwenTE通用模型阶段式提示词生成器(QwenTE阶段式提示词生成器):
    ...
    def INPUT_TYPES(cls):
        ...
        data["required"] = OrderedDict({"通用模型": (any_type,)})
