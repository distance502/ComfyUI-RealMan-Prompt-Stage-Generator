...
- 所有模式统一输出自然语言提示词；简洁模式也会保留 NSFW、运行时档案和用户显式细节，标准/详细模式生成分层叙事描述。主提示词、智能文本、标签块、角色设定图和模型后置都会在稳定化与最终避重后再次执行自然语言合同校验，散装标签串或语言不符时自动回退到自然语言 Skill 结果。
- 本地 GGUF、API 接口和仅 Skill 三种生成路径。
- 参考图片反推与角色设定图工作流。
- 标签块编排、用户标签库和预设/历史管理。
- 本机 Edge/Chromium 实时内嵌网页浏览、备用完整浏览器与显式提示词联网搜索；完整提示词可回填当前节点、复制，或拆分后加入标签库。
- 可选 NSFW 工作台提供 26 个结构化字段、569 个内置选项条目、16 套预设和 7 套负面方案，并支持预览、写回与负面提示词同步。
...
主浏览区不是 iframe。后端会启动隔离的本机 Microsoft Edge/Chrome/Chromium 无头会话，把实时网页画面传回节点，并把鼠标点击、拖动、滚轮、常用按键、粘贴和中文输入发送到真实浏览器页面。因此 GitHub、Google、百度等拒绝 iframe 的网站也不再受 `X-Frame-Options` 或 CSP 嵌入限制，返回、前进和刷新读取的是真实浏览器历史。截图会按显示区域压缩，静止页面自动降低刷新频率，操作后再短时加速。
