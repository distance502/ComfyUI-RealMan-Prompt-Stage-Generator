    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"NSFW结果合同：{'、'.join(dict(settings.get('NSFW结果合同', {}) or {}).get('markers', []) or []) or '未触发'} / {'已固定接触点、人物数量、镜头轴线与结束状态' if dict(settings.get('NSFW结果合同', {}) or {}).get('enabled') else '无'}",
            f"风格隔离策略：{settings.get('风格隔离策略', '平衡收敛')}",
            f"模板风格Skill：{settings.get('模板风格Skill名称', '') or dict(settings.get('模板风格Skill', {}) or {}).get('name', template_style)} / {dict(settings.get('模板风格Skill', {}) or {}).get('base_style', '') or '未解析'}",
...
def build_json_payload(
    *,
    ...
    return {
        ...
        "nsfw_result_contract": dict(settings.get("NSFW结果合同", {}) or {}),
        "video_prompt_result_contract": dict(settings.get("视频提示词结果合同", {}) or {}),
        ...
        "prompt_skill_context": dict(settings.get("提示词Skill上下文", {}) or {}),
        "prompt_skill_summary": str(settings.get("提示词Skill摘要", "") or ""),
        "template_style_skill": dict(settings.get("模板风格Skill", {}) or {}),
        "user_selected_tag_skill": dict(settings.get("用户标签Skill", {}) or {}),
        "intelligence_profile": dict(settings.get("智能编排档案", {}) or {}),
        "task_intent": dict(settings.get("智能任务意图", {}) or {}),
        ...
        "model_call_skill_version": str(settings.get("模型调用Skill版本", "") or ""),
        "model_call_skill_status": str(settings.get("模型调用Skill状态", "") or ""),
        "model_call_skill_channel": str(settings.get("模型调用Skill通道", "") or ""),
        "model_call_skill_error": str(settings.get("模型调用Skill错误", "") or ""),
