...
_RUNTIME_RANDOM_AUTO_SLOT_BUDGET = 10
...
def _allowed_random_additions(current_count: int, slot_count: int, intensity: str) -> int:
    remaining = max(0, min(_RUNTIME_RANDOM_AUTO_SLOT_BUDGET, int(slot_count)) - int(current_count))
    if remaining <= 0:
        return 0
    if str(intensity).strip() == "弱":
        return min(1, remaining)
    if str(intensity).strip() == "中":
