...
3. 中文成品必须为 800-1200 字自然语言；英文输出目标 420-560 words；中英模式先输出英文提示词，再用一小段中文说明总结视觉方向。内容必须精细，但不要用近义词反复堆叠凑字数。
...
def build_smart_text_seed(
    *,
    ...
    if language == "纯英文":
        language_instruction = "语言要求：只输出英文正向提示词，目标 420-560 words，不要包含中文解释。"
    elif language == "英文提示词+中文说明":
        language_instruction = "语言要求：先输出英文正向提示词，目标 420-560 words；末尾追加一小段中文说明，以“中文说明：”开头，说明画面主线。"
    else:
        language_instruction = "语言要求：只输出中文正向提示词，必须为 800-1200 字自然语言。"
    output_instruction = (
        "输出：当前是非人物主体，优先吸收基础提示词和当前主线，写清处境与功能目标、事件触发、主体状态变化、功能或运动回应、局势升级、环境反馈、光影迁移、视觉高潮与最后定格；聚焦物体、机械、建筑、场景或概念设计本身，只补结构、材质、尺度、功能部件和空间关系；中文必须为 800-1200 字自然语言/英文目标 420-560 words；不要原样抄写标签，不要锁死风格和景别，不要重复堆同义标签。"
        if non_person
        else "输出：优先吸收基础提示词和当前主线，写清处境、人物动机、事件触发、动作回应、局势升级、情绪转折、环境与光线反馈、视觉高潮以及镜头最终定格；中文必须为 800-1200 字自然语言/英文目标 420-560 words；默认完整人物入镜，除非用户明确要求近景、特写或半身；不要原样抄写标签，不要锁死风格和景别，不要重复堆同义标签，语气像具有剧情的电影剧照说明，不像标签列表。"
    )
    return "\n".join(
        [
            f"语言：{language}",
